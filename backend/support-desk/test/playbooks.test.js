...
describe("bundled playbook copy", () => {
  it("has no em dashes in customer-facing bodies", async () => {
    const { DEFAULT_PLAYBOOKS } = await import("../src/playbooks.js");
    for (const playbook of DEFAULT_PLAYBOOKS) {
      assert.equal(
        playbook.body.includes("\u2014") || playbook.body.includes("\u2013"),
        false,
        `${playbook.id} body contains an em or en dash`
      );
    }
  });
});
