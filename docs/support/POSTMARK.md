...
`in.benandbill.app` is already on the bills server (see `backend/email-in/README.md`).
You cannot attach the same domain to a second server.
...
**Preferred (no extra DNS): Gmail → Postmark hash address**

While Gmail forwarding is configured (another agent), forward
`support@benandbill.app` to the support server's default inbound address:
...
After `npx wrangler deploy` from `backend/support-desk`:

```
https://ben-support-desk.luke-9e7.workers.dev/inbound?secret=<WEBHOOK_SECRET>
```

Replace `luke-9e7` with your Cloudflare workers.dev subdomain if different.

Set the secret once:

```bash
cd backend/support-desk
openssl rand -hex 32 | tee .webhook-secret.local   # gitignored — do not commit
npx wrangler secret put WEBHOOK_SECRET             # paste the same value
```
...
4. Choose inbound mail path:
   - **Gmail forward** → `<InboundHash>@inbound.postmarkapp.com`, or
...
6. Copy the **Server API token** for outbound replies →
   `npx wrangler secret put POSTMARK_SERVER_TOKEN` (never commit).
...
| `backend/email-in/.webhook-secret.local` | Bills pipe only — do not reuse for support |
...
curl -s -X POST "https://ben-support-desk.luke-9e7.workers.dev/inbound?secret=${SECRET}" \
...
# expect: ok (once worker is deployed)
```
...
- Bills inbound: `backend/email-in/README.md`
